...
clients never send a space id.

| Endpoint | Auth | Request | Response |
| --- | --- | --- | --- |
| `POST /v1/space` | — | `{name?}` | `{space_id, device_id, device_token, secret}` |
| `POST /v1/join` | — | `{join_hash, name?}` | `{device_id, device_token}` |
| `POST /v1/devices/join-secret` | ✓ | `{}` | `{secret}` |
...
| `POST /v1/sync/pull` | ✓ | `{cursor?, limit?}` | `{cursor, patches: [{space, table, pk, version, op, deleted, body}]}` |
| `POST /v1/sync/push` | ✓ | `{mutations: [{table, pk, version?, body?, op}]}` | `{results: [{table, pk, version}]}` |
...
- **Last-write-wins**: rows are keyed by `(space_id, table, pk)`; a push
  ...
  rejected**, however stale the client's `version` field (it is advisory and
...
- Pairing: `POST /v1/space` returns the first device's token plus a pairing
  secret; any existing device mints a fresh one with
  `POST /v1/devices/join-secret`. The secret embeds the space id so the relay
  can attribute join attempts; only SHA-256 of the full credential is stored.
...
- `sync_rows(space_id, table_name, pk, body, version, deleted, updated_at)`
...
EXISTS`); there are no numbered migrations to maintain.
