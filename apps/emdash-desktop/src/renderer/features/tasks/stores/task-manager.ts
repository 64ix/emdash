...
export class TaskManagerStore {
  ...
  constructor(
    ...
    // caller (the board sync service's PR-facts derivation, the provisioned
    // `implementing` hook, and the inbound issues sync deriving facts from
    // GitHub), not just renderer-initiated RPCs, mirroring the
    // `taskProvisionedChannel` "regardless of which path" precedent above.
    this._unsubWorkflowStageUpdated = events.on(
      ...
      ({ taskId, projectId: evtProjectId, stage }) => {
        ...
        if (store && isRegistered(store)) {
          runInAction(() => {
            store.data.workflowStage = stage ?? undefined;
          });
        }
      }
    );
