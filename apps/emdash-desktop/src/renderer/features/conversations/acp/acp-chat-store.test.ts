...
describe('AcpChatStore.search', () => {
  ...
  it('searchHistoryExhausted keeps updating once observed, instead of caching its first evaluation (the #34 staleness bug shape)', async () => {
    ...
    store.session = {
      ...
      // `sessionState` is not optional on the real session type: since ticket
      // #33, `_loadOlderHistory`'s sync block also runs `_syncAttentionQueue`,
      // which reads `sessionState.current().pendingPermissions`. Omitting it
      // makes that read throw *inside* `_loadOlderHistory`'s own try/catch,
      // which silently aborts the page load and never reaches `_syncSearch`.
      sessionState: { current: () => ({ pendingPermissions: [] }) },
    } as never;
    store.bindView(null);
  ...
  it('an older-history page load recomputes results immediately, without waiting on the debounce window', async () => {
    ...
    store.session = {
      ...
      // See the preceding test: `_syncAttentionQueue` (#33) reads this during
      // `_loadOlderHistory`'s sync block.
      sessionState: { current: () => ({ pendingPermissions: [] }) },
    } as never;
    store.bindView(null);
    store.openSearch();
