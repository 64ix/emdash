...
-- distinguishable. The INSERT triggers are recreated with the same guarantee:
-- a fresh row is stamped with the max of the wall clock, one more than any
-- clock the engine recorded for that row in sync_row_state (so a delete +
-- same-millisecond re-create of a row never collides with the recorded
-- "applied/acked" clock — the engine would otherwise mistake the new row for
-- an untouched copy and never push it), and one more than the table's
-- `sync:lastPushed:<table>` watermark (so a row created in the same
-- millisecond as a pushed sibling still satisfies `sync_ts > lastPushed`).
-- The engine's sync_row_state side table records the clock value observed at
-- apply/ack time so an applied remote row is never mistaken for a dirty local
-- edit (the trigger re-stamp loop trap). DELETE triggers feed sync_tombstones,
-- which the engine drains as relay tombstones; the engine clears the entries
-- its own tombstone applies create (DELETE on a missing row fires nothing).
--
...
--> statement-breakpoint
CREATE TRIGGER `trg_projects_sync_ts_ins` AFTER INSERT ON `projects` BEGIN UPDATE `projects` SET `sync_ts` = MAX(CAST((unixepoch('subsec') * 1000) AS INTEGER), COALESCE((SELECT `row_sync_ts` + 1 FROM `sync_row_state` WHERE `table_name` = 'projects' AND `pk` = `NEW`.`id`), 0), COALESCE((SELECT CAST(COALESCE(json_extract(`value`, '$'), 0) AS INTEGER) + 1 FROM `kv` WHERE `key` = 'sync:lastPushed:projects'), 0)) WHERE `id` = NEW.`id`; END;
...
CREATE TRIGGER `trg_project_remotes_sync_ts_ins` AFTER INSERT ON `project_remotes` BEGIN UPDATE `project_remotes` SET `sync_ts` = MAX(CAST((unixepoch('subsec') * 1000) AS INTEGER), COALESCE((SELECT `row_sync_ts` + 1 FROM `sync_row_state` WHERE `table_name` = 'project_remotes' AND `pk` = json_array(`NEW`.`project_id`, `NEW`.`remote_name`)), 0), COALESCE((SELECT CAST(COALESCE(json_extract(`value`, '$'), 0) AS INTEGER) + 1 FROM `kv` WHERE `key` = 'sync:lastPushed:project_remotes'), 0)) WHERE `project_id` = NEW.`project_id` AND `remote_name` = NEW.`remote_name`; END;
...
CREATE TRIGGER `trg_project_settings_sync_ts_ins` AFTER INSERT ON `project_settings` BEGIN UPDATE `project_settings` SET `sync_ts` = MAX(CAST((unixepoch('subsec') * 1000) AS INTEGER), COALESCE((SELECT `row_sync_ts` + 1 FROM `sync_row_state` WHERE `table_name` = 'project_settings' AND `pk` = `NEW`.`project_id`), 0), COALESCE((SELECT CAST(COALESCE(json_extract(`value`, '$'), 0) AS INTEGER) + 1 FROM `kv` WHERE `key` = 'sync:lastPushed:project_settings'), 0)) WHERE `project_id` = NEW.`project_id`; END;
...
CREATE TRIGGER `trg_tasks_sync_ts_ins` AFTER INSERT ON `tasks` BEGIN UPDATE `tasks` SET `sync_ts` = MAX(CAST((unixepoch('subsec') * 1000) AS INTEGER), COALESCE((SELECT `row_sync_ts` + 1 FROM `sync_row_state` WHERE `table_name` = 'tasks' AND `pk` = `NEW`.`id`), 0), COALESCE((SELECT CAST(COALESCE(json_extract(`value`, '$'), 0) AS INTEGER) + 1 FROM `kv` WHERE `key` = 'sync:lastPushed:tasks'), 0)) WHERE `id` = NEW.`id`; END;
...
CREATE TRIGGER `trg_conversations_sync_ts_ins` AFTER INSERT ON `conversations` BEGIN UPDATE `conversations` SET `sync_ts` = MAX(CAST((unixepoch('subsec') * 1000) AS INTEGER), COALESCE((SELECT `row_sync_ts` + 1 FROM `sync_row_state` WHERE `table_name` = 'conversations' AND `pk` = `NEW`.`id`), 0), COALESCE((SELECT CAST(COALESCE(json_extract(`value`, '$'), 0) AS INTEGER) + 1 FROM `kv` WHERE `key` = 'sync:lastPushed:conversations'), 0)) WHERE `id` = NEW.`id`; END;
...
CREATE TRIGGER `trg_automations_sync_ts_ins` AFTER INSERT ON `automations` BEGIN UPDATE `automations` SET `sync_ts` = MAX(CAST((unixepoch('subsec') * 1000) AS INTEGER), COALESCE((SELECT `row_sync_ts` + 1 FROM `sync_row_state` WHERE `table_name` = 'automations' AND `pk` = `NEW`.`id`), 0), COALESCE((SELECT CAST(COALESCE(json_extract(`value`, '$'), 0) AS INTEGER) + 1 FROM `kv` WHERE `key` = 'sync:lastPushed:automations'), 0)) WHERE `id` = NEW.`id`; END;
...
CREATE TRIGGER `trg_app_settings_sync_ts_ins` AFTER INSERT ON `app_settings` BEGIN UPDATE `app_settings` SET `sync_ts` = MAX(CAST((unixepoch('subsec') * 1000) AS INTEGER), COALESCE((SELECT `row_sync_ts` + 1 FROM `sync_row_state` WHERE `table_name` = 'app_settings' AND `pk` = `NEW`.`key`), 0), COALESCE((SELECT CAST(COALESCE(json_extract(`value`, '$'), 0) AS INTEGER) + 1 FROM `kv` WHERE `key` = 'sync:lastPushed:app_settings'), 0)) WHERE `key` = NEW.`key`; END;
...
CREATE TRIGGER `trg_kv_sync_ts_ins` AFTER INSERT ON `kv` BEGIN UPDATE `kv` SET `sync_ts` = MAX(CAST((unixepoch('subsec') * 1000) AS INTEGER), COALESCE((SELECT `row_sync_ts` + 1 FROM `sync_row_state` WHERE `table_name` = 'kv:prompt-library' AND `pk` = `NEW`.`key`), 0), COALESCE((SELECT CAST(COALESCE(json_extract(`value`, '$'), 0) AS INTEGER) + 1 FROM `kv` WHERE `key` = 'sync:lastPushed:kv:prompt-library'), 0)) WHERE `key` = NEW.`key`; END;
