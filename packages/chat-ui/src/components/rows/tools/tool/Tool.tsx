 ...
 * never interpreted as markup. The one exception is the fetch resource link,
 * which is routed through the same `classifyLink` + fail-closed anchor
 * pattern used by markdown/resource-link rows (see Prose.tsx) rather than a
 * bare `window.open`.
 *
...
/**
 * One affected-resource row. `url` resources reuse the classifyLink +
 * fail-closed anchor pattern (see Prose.tsx) instead of a bare `window.open`;
 * `workspace-file` resources open directly through the existing editor
 * command. Neither variant interprets the resource label as markup.
 */
function ToolResourceRow(props: { resource: ToolResource; itemId: string; rowH: number }) {
  const commands = useCommands();

  if (props.resource.kind === 'workspace-file') {
    ...
    return (
      <button
        ...
        title={path}
        onClick={() =>
          commands().onOpenFile?.({ path, itemId: props.itemId, source: 'resource-link' })
        }
      >
  ...
  const classification = () => commands().classifyLink?.(url);
  const handleClick = (e: MouseEvent) => {
    ...
    // else: browser follows the <a> normally — external-link confirmation and
    // the main-process fail-closed window-open policy own this path.
  };
